...
router.post('/seed', async (req, res) => {
    try {
        console.log("--- 🌱 MEGA-ADATBÁZIS FELTÖLTÉSE (Admin megőrzése) ---");

        // 1. Megkeressük az admint, hogy legyen kire hivatkozni a szolgáltatásoknál
        const admin = await User.findOne({ role: 'ADMIN' });
        if (!admin) {
            return res.status(404).json({ success: false, message: "Nincs admin felhasználó a rendszerben, nem lehet seedelni!" });
        }

        // 2. TISZTÍTÁS (Csak nem-admin felhasználókat törlünk)
        await User.deleteMany({ role: { $ne: 'ADMIN' } });
        await Service.deleteMany({});
        await Appointment.deleteMany({});
        await Record.deleteMany({});

        // 3. Felhasználók (Admin nélkül!)
        const usersData = [
        ...
        const [doc1, doc2, doc3, pat1, pat2, pat3] = createdUsers;

        // 4. Szolgáltatások (Az admin változót használjuk a created_by mezőnél)
        const servicesData = [
        ...
        // 5. Időpontok és Rekordok (Ugyanaz a logika, mint eddig)
        const now = new Date();
        ...
        const recordsData = [
            { patient: pat1._id, doctor: doc1._id, service: createdServices[0]._id, appointment_id: createdAppointments[0]._id, description: "Vizsgálat sikeresen lezajlott.", createdAt: new Date(now.getTime() - 86400000 * 5) },
            { patient: pat1._id, doctor: doc2._id, service: createdServices[2]._id, appointment_id: createdAppointments[1]._id, description: "Speciális cipőbetét javasolt.", createdAt: new Date(now.getTime() - 86400000 * 2) }
        ];
        ...
        res.status(201).json({ success: true, message: "Mega-Seed sikeres, adminok érintetlenek maradtak." });
    } catch (error) {
